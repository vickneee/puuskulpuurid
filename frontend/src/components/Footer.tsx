...
// Optional Facebook link read at build time from Vite env var
const FB_URL = import.meta.env.VITE_FACEBOOK_URL ?? "";
  ...
  return (
    <footer className="py-10 px-6 border-t border-border">
      <div className="max-w-6xl mx-auto grid grid-cols-1 gap-4 sm:grid-cols-3 sm:items-center">
        <Link
          to="/admin/login"
          className="flex items-center justify-center sm:justify-start gap-1.5 font-body text-xs text-muted-foreground/60 hover:text-muted-foreground transition-colors"
        >
